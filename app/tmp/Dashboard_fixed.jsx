...
class ErrorBoundary extends React.Component {
  ...
  componentDidCatch(error, info) { console.error('[Dashboard] Caught error:', error, info); }
  render() {
    if (this.state.hasError) {
      return (
        <div style={{ padding: 32, textAlign: 'center' }}>
          <div style={{ fontSize: 40, marginBottom: 12 }}>⚠️</div>
          <div style={{ fontSize: 16, fontWeight: 700, color: '#f87171', marginBottom: 8 }}>Something went wrong loading the dashboard</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 20 }}>
        ...
        <Link to="/Vehicles" style={{ textDecoration: 'none' }}>
          <div style={{ background: 'linear-gradient(135deg,rgba(96,165,250,0.1),rgba(59,130,246,0.05))', border: '1px solid rgba(96,165,250,0.25)', borderRadius: 16, padding: 18, cursor: 'pointer' }}>
            ...
            <div style={{ fontSize: 12, color: '#7c6a9e', marginTop: 2 }}>{vehicles.length} vehicle{vehicles.length !== 1 ? 's' : ''} saved</div>
          </div>
        </Link>
      </div>
      ...
      <div style={{ background: '#18122b', border: '1px solid #2a1f4a', borderRadius: 20, overflow: 'hidden', marginBottom: 16 }}>
        ...
        {reports.length === 0 ? (
          ...
          reports.slice(0, 5).map(r => {
            ...
            const color = sevColors[r.severity] || '#fbbf24';
            return (
              <div key={r.id} style={{ padding: '14px 20px', borderBottom: '1px solid #231849', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
                <div style={{ minWidth: 0 }}>
                  ...
                  {r.plain_english_explanation && (
                    <div style={{ fontSize: 12, color: '#666', marginTop: 2, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{r.plain_english_explanation}</div>
                  )}
                </div>
                <span style={{ fontSize: 11, fontWeight: 700, color, background: `${color}18`, border: `1px solid ${color}30`, borderRadius: 20, padding: '2px 10px', whiteSpace: 'nowrap' }}>
                  {r.severity || 'N/A'}
                </span>
              </div>
            );
