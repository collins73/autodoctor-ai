...
function exportDiagnosisPDF(diagnosis, vehicle) {
  const date = new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' });
  const vehicleStr = vehicle ? `${vehicle.year} ${vehicle.make} ${vehicle.model}${vehicle.mileage ? ' · ' + Number(vehicle.mileage).toLocaleString() + ' mi' : ''}` : 'N/A';
  const html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Rebel Auto Agent — Diagnostic Report</title>
  <style>
    body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; background: #0d0a1a; color: #f0eeff; margin: 0; padding: 32px; }
    .header { border-bottom: 2px solid #7c3aed; padding-bottom: 16px; margin-bottom: 24px; }
    .logo { font-size: 22px; font-weight: 900; background: linear-gradient(135deg,#fb923c,#f87171); -webkit-background-clip: text; -webkit-text-fill-color: transparent; }
    .date { font-size: 12px; color: #666; margin-top: 4px; }
    .code { font-size: 36px; font-weight: 900; letter-spacing: 4px; color: #f0eeff; margin-bottom: 4px; }
    .desc { font-size: 16px; color: #c4b5fd; font-weight: 700; margin-bottom: 12px; }
    .sev { display: inline-block; padding: 4px 14px; border-radius: 20px; font-size: 12px; font-weight: 700; margin-bottom: 20px; }
    .section { background: #18122b; border-radius: 12px; padding: 16px; margin-bottom: 14px; }
    .section-title { font-size: 10px; font-weight: 700; color: #7c6a9e; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 8px; }
    .section-body { font-size: 14px; color: #a0a0b8; line-height: 1.6; }
    .cost { font-size: 24px; font-weight: 900; color: #4ade80; }
    .warn { background: rgba(248,113,113,0.1); border: 1px solid rgba(248,113,113,0.3); border-radius: 10px; padding: 12px; margin-bottom: 14px; }
    .warn-title { font-size: 11px; font-weight: 700; color: #f87171; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 6px; }
    .action { background: rgba(74,222,128,0.07); border: 1px solid rgba(74,222,128,0.25); border-radius: 10px; padding: 12px; margin-bottom: 14px; }
    .action-title { font-size: 11px; font-weight: 700; color: #4ade80; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 6px; }
    .footer { margin-top: 32px; padding-top: 16px; border-top: 1px solid #2a1f4a; font-size: 11px; color: #444; text-align: center; }
  </style></head><body>
  <div class="header">
    <div class="logo">⚡ Rebel Auto Agent</div>
    <div class="date">Diagnostic Report · ${date}</div>
  </div>
  <div style="margin-bottom:20px">
    <div style="font-size:11px;color:#7c6a9e;text-transform:uppercase;letter-spacing:1px;margin-bottom:4px">Vehicle</div>
    <div style="font-size:15px;font-weight:700;color:#e2d9f3">${vehicleStr}</div>
  </div>
  <div class="code">${diagnosis.fault_code || 'N/A'}</div>
  <div class="desc">${diagnosis.fault_description || ''}</div>
  <span class="sev" style="background:rgba(168,85,247,0.15);color:#c4b5fd;border:1px solid rgba(168,85,247,0.3)">${diagnosis.severity || 'Medium'}</span>
  <div class="section">
    <div class="section-title">Plain English Explanation</div>
    <div class="section-body">${diagnosis.plain_english_explanation || 'N/A'}</div>
  </div>
  <div class="section">
    <div class="section-title">Estimated Repair Cost</div>
    <div class="cost">$${diagnosis.estimated_cost_low ?? '?'} – $${diagnosis.estimated_cost_high ?? '?'}</div>
  </div>
  ${diagnosis.consequences_if_ignored ? `<div class="warn"><div class="warn-title">⚠️ If Ignored</div><div style="font-size:13px;color:#fca5a5">${diagnosis.consequences_if_ignored}</div></div>` : ''}
  ${diagnosis.recommended_action ? `<div class="action"><div class="action-title">✅ Recommended Action</div><div style="font-size:13px;color:#86efac">${diagnosis.recommended_action}</div></div>` : ''}
  <div class="footer">Generated by Rebel Auto Agent · rebelauto-diagnostics-ai.base44.app</div>
  </body></html>`;
  const blob = new Blob([html], { type: 'text/html' });
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url;
  a.download = `rebel-auto-${diagnosis.fault_code || 'report'}-${Date.now()}.html`;
  a.click();
  URL.revokeObjectURL(url);
}

function shareDiagnosis(diagnosis, vehicle) {
  const vehicleStr = vehicle ? `${vehicle.year} ${vehicle.make} ${vehicle.model}` : 'my vehicle';
  const text = `🔧 Rebel Auto Agent diagnosed ${vehicleStr}\n\nFault Code: ${diagnosis.fault_code}\n${diagnosis.fault_description}\n\nEst. Repair: $${diagnosis.estimated_cost_low}–$${diagnosis.estimated_cost_high}\n\nDiagnosed with Rebel Auto Agent ⚡`;
  if (navigator.share) {
    navigator.share({ title: 'My Diagnostic Report', text }).catch(() => {});
  } else {
...
function DiagnosisCard({ diagnosis, trialInfo, vehicle }) {
  ...
  return (
    <div style={{ background: '#120f22', border: `1px solid ${color}40`, borderRadius: 16, padding: 20, marginBottom: 16 }}>
      ...
      {/* ── Share / Export ── */}
      <div style={{ display: 'flex', gap: 10, marginTop: 16, paddingTop: 14, borderTop: '1px solid #2a1f4a' }}>
        <button onClick={() => exportDiagnosisPDF(diagnosis, vehicle)}
          style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, background: 'linear-gradient(135deg,rgba(124,58,237,0.15),rgba(168,85,247,0.08))', border: '1px solid rgba(168,85,247,0.3)', borderRadius: 10, padding: '10px 0', color: '#c4b5fd', fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>
          📄 Export Report
        </button>
        <button onClick={() => shareDiagnosis(diagnosis, vehicle)}
          style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, background: 'linear-gradient(135deg,rgba(96,165,250,0.1),rgba(59,130,246,0.05))', border: '1px solid rgba(96,165,250,0.25)', borderRadius: 10, padding: '10px 0', color: '#93c5fd', fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>
          🔗 Share
        </button>
      </div>
    </div>
...
Please confirm this diagnosis and answer any follow-up questions.`;
  ...
  return (
    <div>
      ...
      {step === 4 && (
        <div>
          <DiagnosisCard diagnosis={diagnosis} trialInfo={trialInfo} vehicle={vehicle} />
