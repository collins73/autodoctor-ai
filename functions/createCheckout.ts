...
Deno.serve(async (req) => {
  try {
    ...
    // Try to get user from auth — but don't block if unavailable (app may be public-facing)
    let userId = '';
    let userEmail = '';
    try {
      ...
      // Auth failed — proceed as anonymous checkout
    }

    // Also accept userId/email from request body as fallback
