export default async function handler(req: any, context: any) {
  ...
  if (!accessToken) {
    return { ok: false, error: "LinkedIn token not found" };
  }

  const postRotation = [
    `🚗💡 Just launched Rebel Auto Agent — a free AI tool that explains your check engine code in plain English.
...
#AutoRepair #DIYCars #SaveMoney #CarTech`,
    ...
    `Building autonomous systems for car owners 🔧⚡

Just shipped Rebel Auto Agent — an AI diagnostic tool that reads OBD-II codes and tells you what's wrong with your car in seconds.
...
#BuildInPublic #AI #Automotive #SideProject`,
    ...
    `Problem: Check engine light. No idea what it means. Shop wants $150 just to look at it.
...
#CarOwner #AutoRepair #FrugalLiving`,
    ...
    `Just shipped a full-stack AI car diagnostic platform 🚗⚡
...
#WebDevelopment #AI #Automotive #Startup`,
    ...
    `🙋 Rebel Auto Agent is live and I need your feedback.
...
#ProductFeedback #BuildInPublic`,
  ...
  try {
    ...
    if (!meResponse.ok) {
      return {
        ok: false,
        error: `Failed to get LinkedIn profile: ${meResponse.statusText}`,
      };
    }
    ...
    // Rotate post based on day
    const dayOfMonth = new Date().getDate();
    const postIndex = dayOfMonth % postRotation.length;
    const postContent = postRotation[postIndex];
    ...
    if (!postResponse.ok) {
      ...
      return {
        ok: false,
        error: `LinkedIn post failed: ${JSON.stringify(error)}`,
      };
    }

    return {
      ok: true,
      message: `Posted to LinkedIn (rotation index: ${postIndex})`,
    };
  } catch (e: any) {
