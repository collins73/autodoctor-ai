...
export default function Diagnose() {
  ...
  const canDiagnose = trialInfo?.can_diagnose !== false; // default true if not loaded yet
  ...
  const runDiagnostic = async () => {
    ...
    // Hard gate — check trial before running
    if (!canDiagnose) {
    ...
    try {
      ...
      const prompt = `Vehicle: ${vehicleDesc}${vehicle.mileage ? `, ${vehicle.mileage} miles` : ''}${vehicle.location ? `, located in ${vehicle.location}` : ''}.
Fault code: ${faultCode.toUpperCase()}.
Please diagnose this fault code: explain what it means in plain English, the risk if ignored, estimated repair cost range, and recommended action.`;
  ...
  const reset = () => {
    setStep(1); setVehicle({ year: '', make: '', model: '', mileage: '', name: '', location: '' });
    setFaultCode(''); setConversationId(null); setMessages([]); setInput('');
    setError(null); setSelectedVehicleId(''); conversationRef.current = null;
    fetchTrialStatus(); // refresh trial status on reset
  };

  const stepDone = (n) => step > n;
  const stepActive = (n) => step === n;

  // PAYWALL GATE — shown when trial is exhausted and not pro
  ...
  return (
    <div>
      {/* Trial usage bar — shown when on trial and not exhausted */}
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 0, marginBottom: 28 }}>
        {[1, 2, 3, 4].map((n, i) => (
          <React.Fragment key={n}>
            ...
            {i < 3 && (
              <div style={{ width: 40, height: 2, background: stepDone(n) ? '#22c55e' : '#2a1f4a', flexShrink: 0, margin: '0 4px 16px' }} />
            )}
          </React.Fragment>
        ))}
      </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12, marginBottom: 12 }}>
              {[['Year', 'year', '2021'], ['Make', 'make', 'Toyota'], ['Model', 'model', 'Camry']].map(([lbl, key, ph]) => (
                <div key={key}>
                  ...
                  <input style={S.input} placeholder={ph} value={vehicle[key]} onChange={e => setVehicle(p => ({ ...p, [key]: e.target.value }))} />
                </div>
              ))}
            </div>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
          <div style={S.card}><div style={S.cardPad}>
            ...
            <input
              ...
              placeholder="P0300" maxLength={6}
              value={faultCode} onChange={e => setFaultCode(e.target.value.toUpperCase())}
            />
      ...
      {step === 4 && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
          <div style={S.card}>
            <div style={{
              ...
              <div>
                <div style={{
                  ...
                  background: 'linear-gradient(135deg,#a855f7,#c084fc)', WebkitBackgroundClip: 'text',
                  WebkitTextFillColor: 'transparent', backgroundClip: 'text'
                }}>{faultCode}</div>
                <div style={{ fontSize: 13, color: '#ccc', marginTop: 2 }}>
                  {vehicle.year} {vehicle.make} {vehicle.model}{vehicle.location ? ` · ${vehicle.location}` : ''}
                </div>
              </div>
            ...
            <div style={{ padding: '20px 24px', minHeight: 300, maxHeight: 480, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 16 }}>
              <AnimatePresence>
                {messages.map((msg, i) => (
            ...
            <div style={{ padding: '16px 24px', borderTop: '1px solid #2a1f4a' }}>
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                <button
                  ...
                  style={{
                    ...S.btnPrimary, width: 48, height: 48, padding: 0, borderRadius: 12, flexShrink: 0,
